    ...
    <div className="overflow-hidden rounded-2xl border border-border bg-background shadow-sm">
      <div className="grid grid-cols-[72px_minmax(0,1fr)] gap-2.5 p-2.5 sm:grid-cols-[82px_1fr] sm:gap-3">
        <img
          ...
          alt={tc(product.name)}
          className="h-[76px] w-full rounded-xl object-cover sm:h-24"
        />
        <div className="min-w-0 py-1">
          ...
          <div className="mt-2 flex gap-1.5">
            <Link
              ...
              className="inline-flex items-center gap-1 rounded-full border border-border px-2.5 py-1.5 text-[9px] tracking-[0.12em] uppercase"
            >
              Ver <ArrowRight className="h-3 w-3" />
            </Link>
            <button
              onClick={() => onAdd(product)}
              className="inline-flex items-center gap-1 rounded-full bg-primary px-2.5 py-1.5 text-[9px] tracking-[0.1em] text-primary-foreground uppercase"
            >
    ...
    setMessages([
      {
        id: nextId.current++,
        role: "florencio",
        text: "¡Hola! Soy Florencio. Estoy aquí para ayudarte a encontrar el detalle perfecto.",
      },
      {
        id: nextId.current++,
        role: "florencio",
  ...
  const send = async (raw: string) => {
    ...
    try {
      ...
      const reply = parts.length
        ? `Perfecto. ${parts.join(" · ")}. Voy a cruzarlo con nuestro catálogo real y mostrarte lo que mejor encaje.`
        : "Perfecto. Voy a tomar eso como punto de partida y cruzarlo con el catálogo real de Deluxury.";
  ...
  const addRecommended = (product: Product) => {
    ...
    setMessages((prev) => [
      ...prev,
      {
        id: nextId.current++,
        role: "florencio",
        text: `Listo. Añadí ${product.name} a tu carrito. 🛒 Puedo ayudarte a completar el detalle si quieres.`,
      },
    ]);
  };
    ...
    <>
      ...
      >
        <header className="flex shrink-0 items-center justify-between border-b border-border bg-secondary/55 px-4 py-3.5 sm:px-5 sm:py-4">
          ...
          <button onClick={() => setOpen(false)} aria-label="Cerrar chat" className="rounded-full border border-border p-2 hover:border-primary/50">
            <X className="h-4 w-4" />
          </button>
        </header>
        ...
        <div ref={scrollRef} className="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain px-3.5 py-4 sm:px-4 sm:py-5">
          {messages.map((message) => (
            <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
              {message.role === "florencio" ? (
                <div className="flex max-w-[91%] items-start gap-2.5">
                ...
                <div className="max-w-[88%] rounded-2xl rounded-br-md bg-primary px-4 py-3 text-primary-foreground">
          ...
          {thinking && (
            <div className="flex items-center gap-2.5 px-1">
              <FlorencioAvatar small />
              <div className="florencio-thinking" aria-label="Florencio está pensando">
                <span /><span /><span /><i />
              </div>
          ...
          {recommendations.length > 0 && messages.length > 1 && (
            <div className="space-y-2.5 pt-1">
              <div className="flex items-center gap-2 px-1 text-[9px] tracking-[0.18em] text-muted-foreground uppercase">
                <Sparkles className="h-3 w-3 text-primary" /> Mis recomendaciones
              </div>
          ...
          {products.length === 0 && messages.length > 1 && (
            <div className="rounded-2xl border border-border bg-secondary/40 p-4 text-xs leading-relaxed text-muted-foreground">
              El catálogo todavía no tiene productos disponibles para recomendar.
            </div>
          )}
        </div>
        ...
        <div className="shrink-0 border-t border-border bg-background px-3.5 py-3 pb-[max(12px,env(safe-area-inset-bottom))] sm:px-4 sm:py-3 sm:pb-3">
          <div className="mb-2 flex max-w-full gap-2 overflow-x-auto overscroll-x-contain pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {QUICK_PROMPTS.map((prompt) => (
              <button key={prompt} onClick={() => void send(prompt)} className="shrink-0 rounded-full border border-border px-3 py-1.5 text-[9px] tracking-[0.05em] hover:border-primary/50">
                {prompt}
              </button>
            ))}
          </div>
          <form onSubmit={(e) => { e.preventDefault(); void send(input); }} className="flex min-w-0 items-center gap-2 rounded-2xl border border-border bg-secondary/35 p-1.5 focus-within:border-primary/50">
            ...
            <button type="submit" aria-label="Enviar" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary text-primary-foreground disabled:opacity-40" disabled={!input.trim() || thinking}>
              <Send className="h-4 w-4" />
            </button>
          </form>
          <p className="mt-2 text-center text-[8px] tracking-[0.1em] text-muted-foreground uppercase">
            <Bot className="mr-1 inline h-3 w-3" /> Recomendaciones basadas en el catálogo real
          </p>
        </div>
      </aside>
    </>

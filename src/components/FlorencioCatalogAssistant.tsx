...
function parseMedia(raw?: string): MediaItem[] {
  ...
  try {
    ...
    return parsed.filter((x): x is Record<string, unknown> => !!x && typeof x === "object").map((x) => ({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "in") {
        ...
        const { data, error: signUpError } = await supabase.auth.signUp({ email, password, options: { emailRedirectTo: `${window.location.origin}/florencio`, data: { full_name: name } } });
...
function ProductCardMini({ product, profileImage, onAdd }: { product: Product & { matchReasons?: string[] }; profileImage: string; onAdd: (p: Product) => void }) {
  const { currency } = useStore(); const { data: settings } = useQuery(settingsQuery); const tc = useContentTranslator([product.name]);
...
export default function FlorencioCatalogAssistant() {
  const { data: products = [] } = useQuery(productsQuery); const { data: categories = [] } = useQuery(categoriesQuery); const { data: settings } = useQuery(settingsQuery); const { add } = useStore();
  const [session, setSession] = useState<Session | null>(null); const [authReady, setAuthReady] = useState(false); const [activeTab, setActiveTab] = useState<Tab>("chat"); const [mobileMenu, setMobileMenu] = useState(false);
  const [sessionId, setSessionId] = useState<string | null>(null); const [input, setInput] = useState(""); const [loading, setLoading] = useState(false); const [historyLoading, setHistoryLoading] = useState(false);
  ...
  const localSuggestions = useMemo(() => rankFlorencioProducts(products, categories, { keywords: [] }, 4), [products, categories]);

  useEffect(() => { supabase.auth.getSession().then(({ data }) => { setSession(data.session); setAuthReady(true); }); const { data: { subscription } } = supabase.auth.onAuthStateChange((_e, next) => setSession(next)); return () => subscription.unsubscribe(); }, []);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (saved) { const row = saved as unknown as SessionRow; setSessionId(row.id); setMessages(Array.isArray(row.messages) && row.messages.length ? row.messages : []); setFilters(row.filters ?? { keywords: [] }); setRecommendations(rankFlorencioProducts(products, categories, row.filters ?? { keywords: [] }, 4)); }
      setRecommendationHistory((recs ?? []) as RecommendationRow[]); setOrders((orderRows ?? []) as unknown as OrderRow[]); setHistoryLoading(false);
    };
    void load();
  ...
  useEffect(() => { if (!messages.length) { setMessages([{ id: "welcome-1", role: "florencio", text: "Hola. Soy Florencio, tu asistente floral de Deluxury.", createdAt: new Date().toISOString() }, { id: "welcome-2", role: "florencio", text: "Cuéntame para quién es el detalle, la ocasión o tu presupuesto y te guío.", createdAt: new Date().toISOString() }]); } }, [messages.length]);
  useEffect(() => { if (activeTab === "chat" && chatRef.current) chatRef.current.scrollTo({ top: chatRef.current.scrollHeight, behavior: "smooth" }); }, [messages, loading, activeTab]);

  const getRecommendations = (next: FlorencioFilters) => { const ranked = rankFlorencioProducts(products, categories, next, 4); return ranked.length ? ranked : products.filter((p) => p.is_active).slice(0, 4); };
  const persist = async (nextMessages: ChatMessage[], nextFilters: FlorencioFilters, nextRecs: ReturnType<typeof rankFlorencioProducts>, queryText: string) => {
    if (!session?.user.id) return; const payload = { user_id: session.user.id, title: queryText.slice(0, 80), messages: nextMessages.slice(-18), filters: nextFilters, recommendation_product_ids: nextRecs.map((p) => p.id), updated_at: new Date().toISOString() }; let id = sessionId;
    if (id) { const { error } = await supabase.from("florencio_sessions").update(payload).eq("id", id).eq("user_id", session.user.id); if (error) throw error; }
    else { const { data, error } = await supabase.from("florencio_sessions").insert(payload).select("id").single(); if (error) throw error; id = String(data.id); setSessionId(id); }
    if (nextRecs.length && id) await supabase.from("florencio_recommendations").upsert(nextRecs.map((p, i) => ({ user_id: session.user.id, session_id: id, product_id: p.id, query_text: queryText, reason: p.matchReasons?.[0] ?? "Coincide con tu búsqueda.", rank: i + 1 })), { onConflict: "session_id,product_id" });
  };
  ...
  const send = async (raw = input) => {
    ...
    const local = parseFlorencioFilters(text); setInput(""); setLoading(true);
    const userMessage: ChatMessage = { id: uid(), role: "user", text, createdAt: new Date().toISOString() }; const nextMessages = [...messages, userMessage]; setMessages(nextMessages);
    try {
      const result = await askFlorencioAI({ message: text, history: messages.slice(-8).map((m) => ({ role: m.role, text: m.text })), currentFilters: filters });
      const merged: FlorencioFilters = { recipient: result.filters.recipient ?? local.recipient ?? filters.recipient, occasion: result.filters.occasion ?? local.occasion ?? filters.occasion, style: result.filters.style ?? local.style ?? filters.style, color: result.filters.color ?? local.color ?? filters.color, budgetMax: result.filters.budgetMax ?? local.budgetMax ?? filters.budgetMax, keywords: Array.from(new Set([...filters.keywords, ...local.keywords, ...(result.keywords ?? []), ...(result.filters.keywords ?? [])])).slice(0, 20) };
      const nextRecs = getRecommendations(merged); const reply = result.reply.length > 220 ? `${result.reply.slice(0, 217)}…` : result.reply; const assistant: ChatMessage = { id: uid(), role: "florencio", text: reply, createdAt: new Date().toISOString() }; const finalMessages = [...nextMessages, assistant];
      setFilters(merged); setRecommendations(nextRecs); setMessages(finalMessages); await persist(finalMessages, merged, nextRecs, text);
    } catch {
      const merged: FlorencioFilters = { recipient: local.recipient ?? filters.recipient, occasion: local.occasion ?? filters.occasion, style: local.style ?? filters.style, color: local.color ?? filters.color, budgetMax: local.budgetMax ?? filters.budgetMax, keywords: Array.from(new Set([...filters.keywords, ...local.keywords])).slice(0, 20) };
      const nextRecs = getRecommendations(merged); const assistant: ChatMessage = { id: uid(), role: "florencio", text: "Perfecto. Entendí la idea y la estoy cruzando con el catálogo real de Deluxury.", createdAt: new Date().toISOString() }; const finalMessages = [...nextMessages, assistant];
      setFilters(merged); setRecommendations(nextRecs); setMessages(finalMessages); try { await persist(finalMessages, merged, nextRecs, text); } catch {}
  ...
  const tabs: Array<{ id: Tab; label: string; icon: typeof MessageCircle; private?: boolean }> = [
    { id: "chat", label: "Chat", icon: MessageCircle }, { id: "recommendations", label: "Recomendaciones", icon: Sparkles, private: true }, { id: "orders", label: "Mis pedidos", icon: Package, private: true }, { id: "account", label: "Mi cuenta", icon: UserRound }, { id: "gallery", label: "Florencio", icon: ImageIcon },
  ];
  const selectTab = (tab: Tab) => { setActiveTab(tab); setMobileMenu(false); };
  const navItem = (tab: Tab) => { const t = tabs.find((x) => x.id === tab)!; const Icon = t.icon; return <button type="button" onClick={() => selectTab(tab)} className={`flex w-full items-center gap-3 rounded-2xl px-3.5 py-3 text-left text-sm transition ${activeTab === tab ? "bg-primary/10 text-foreground shadow-sm" : "text-muted-foreground hover:bg-secondary/60 hover:text-foreground"}`}><Icon className="h-4 w-4 shrink-0" />{t.label}{t.private && !session ? <LogIn className="ml-auto h-3.5 w-3.5 text-primary" /> : null}</button>; };

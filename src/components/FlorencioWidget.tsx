...
export default function FlorencioWidget() {
  ...
  useEffect(() => {
    const onProductSelected = (event: Event) => {
      ...
      const selected = randomMessage(PRODUCT_MESSAGES);
      show("product", productName ? `${selected} ${productName}` : selected);
    };
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      greetingTimerRef.current = window.setTimeout(() => {
        if (!document.hidden && !visible) show("greeting", randomMessage(GREETING_MESSAGES));
        schedule();
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="absolute -top-1 right-0 flex h-6 w-6 items-center justify-center rounded-full border border-border bg-background/95 text-muted-foreground shadow-sm transition-colors hover:text-foreground"
      >
        <X className="h-3 w-3" />
      </button>
    </div>
